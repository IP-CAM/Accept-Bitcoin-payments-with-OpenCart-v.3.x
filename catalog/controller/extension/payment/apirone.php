...
class ControllerExtensionPaymentApirone extends Controller {
	...
	public function index() {
		...
		if ($test){
			...
			$address = $this->config->get('payment_apirone_merchant'); // Destination Bitcoin address
   ...
   isset($this->request->get['transaction_hash'])
		...
		if($secret == $this->config->get('payment_apirone_secret')) {
			$order_id = (int) $refnum;
		} else {
			$order_id = 0;
		};
		...
		switch($status) {
			case -1:
				...
				break;
			case 0:
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_apirone_canceled_status_id'), $notes, true);
				//$this->response->redirect($cancel_url);
				break;
			case 1:
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_apirone_order_status_id'), $notes, true);
				//$this->response->redirect($success_url);
				break;
			case 2:
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_apirone_pending_status_id'), $notes, true);
